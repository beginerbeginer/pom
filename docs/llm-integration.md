...
pom supports use cases where slides are created from XML generated by LLMs (GPT-4o, Claude, etc.).

## LLM Specification Guide

This section provides a compact specification for having LLMs generate XML in pom format. Include it in your system prompt.
...
`buildPptx` accepts XML strings directly. XML tags (PascalCase) are mapped to POM node types, and attribute values are automatically type-coerced using Zod schemas.
